...
namespace thts {
    ERFixedDepthUctDNode::ERFixedDepthUctDNode(
        shared_ptr<UctManager> thts_manager,
        shared_ptr<const State> state,
        int decision_depth,
        int decision_timestep,
        shared_ptr<const UctCNode> parent) :
            FixedDepthUctDNode(thts_manager, state, decision_depth, decision_timestep, parent)
    {
    }

    double ERFixedDepthUctDNode::compute_ucb_term(int num_visits, int child_visits) const {
        ...
        double ucb_term = pow(num_visits_d, 0.25) / sqrt(child_visits_d);
