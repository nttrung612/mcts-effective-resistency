        ...
        ThtsEnvContext& ctx) 
    {
        ...
        if (!manager.use_avg_return) {
            if (manager.use_max_heap) {
                MentsDNode::backup_soft_with_max_heap(ctx);
            } else {
                num_backups++;
                ActionDistr action_weights;
                double sum_weights;
                double normalisation_term;
                lock_all_children();
                // Explicitly call MentsDNode::compute_action_weights to avoid ER bonus leakage
        ...
        MentsDNode::backup(trial_rewards_before_node, trial_rewards_after_node, trial_cumulative_return_after_node, trial_cumulative_return, ctx);
    }
